...
Description :
--------------------------------------------------
This is a verilog code, to implement a 8-Bit MOD Gray Counter.
...
module Gray_Counter_8_Bit (
    ...
    input  [7:0] Preload_Counter_Value_In,
    input  [7:0] MOD_Value_In,

    output       Counter_Running_Flag_Out,
    output       Counter_Rollover_Flag_Out,
    output [7:0] Counter_Gray_Count_Out
);
...
reg  [7:0] r_Counter_Value = 8'b0;

wire [7:0] w_Counter_Gray_Count;
wire [7:0] w_Preload_Counter_Count;
wire [7:0] w_Adjusted_MOD_Value;
...
// Assignments
// --------------------------------------------------
assign Counter_Gray_Count_Out    = Enable_In ? w_Counter_Gray_Count : 8'bZ;
...
// Other Logic
// --------------------------------------------------
// Binary to Gray Converter 8-Bit
assign w_Counter_Gray_Count[7] = r_Counter_Value[7];
...
// --------------------------------------------------
// Gray_Counter_8_Bit Logic
// --------------------------------------------------
always @ (negedge Clk_In or posedge Reset_In)
    begin
        if (Reset_In)
            begin
                r_Counter_Value    <= 8'b0;
